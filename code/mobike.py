...
train_path = 'tindata/train.csv'
test_path = 'tindata/test.csv'
flag = True

# 相差的分钟数
def diff_of_minutes(time1, time2):
    d = {'5': 0, '6': 31, } #python内建数据类型（key:value）
    try:
...
def get_label(data):
    #result_path = cache_path + 'true.pkl' 
    result_path = cache_path + 'true_%d.pkl' % (time.time() - t0)
    if os.path.exists(result_path):
        true = pickle.load(open(result_path, 'rb+'))
    else:
        train = pd.read_csv(train_path)
...
if __name__ == "__main__":
    ...
    test = pd.read_csv(test_path)    
    print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
          + '划分训练集为train1-5.23之前与train2-5.23之后')
    ...
    test_feat.loc[:,'pred'] = model.predict(xgb.DMatrix(test_feat[predictors]))
